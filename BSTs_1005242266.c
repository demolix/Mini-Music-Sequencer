...
BST_Node *BST_search(BST_Node *root, int bar, double index)
{
	...
	if ((root->bar*10.0)+root->index < (10.0*bar)+index )
	{
...
BST_Node *find_successor(BST_Node *right_child_node)
{
     ...
     ****/
	 ...
	 BST_Node* required=NULL;
	 
	 required = right_child_node;
...
void BST_shiftFreq(BST_Node *root, char note_src[5], char note_dst[5])
{
     ...
     * give you the note names they want. If the user sends a wa
     ...
     ****/
	 double src=0;
	 double dst=0;
	 src = nameToFreq(note_src);
	 dst = nameToFreq(note_dst);
	 if (root != NULL)
	 { 
 ...
 * ******************************************************************************************/
double freqToNewFreq(double frequency, int semitones)
{
	/* This function returns the new frequency after adding
	 * the given semitone. If the frequency is out of bounds 
	 * of array, it returns the frequency unchanged.
	 */
	 for (int i=0; i<100; i++)
	 {
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
    ...
    if (root!=NULL)
	{
		...
		newFreq = freqToNewFreq(root->freq, semitones);
